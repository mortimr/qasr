...
@l1_handler
func warpFromMainnet{
        syscall_ptr : felt*, storage_ptr : Storage*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        from_address : felt, _owner : felt, _l1TokenAddress : felt, _l2TokenAddress : felt,
        _tokenId : felt):
    ...
    mintCredits.write(
        l1TokenAddress=_l1TokenAddress, tokenId=_tokenId, owner=_owner, value=_l2TokenAddress)
...
@external
func consumeMintCredit{
        syscall_ptr : felt*, storage_ptr : Storage*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _l1TokenAddress : felt, _l2TokenAddress : felt, _tokenId : felt):
    let (caller_address) = get_caller_address()
    let (l2TokenAddress) = mintCredits.read(
        l1TokenAddress=_l1TokenAddress, tokenId=_tokenId, owner=caller_address)
    let (mintCreditAvailable) = is_not_zero(l2TokenAddress)

    assert mintCreditAvailable = 1

    IWarpedERC721.createToken(
        contract_address=_l2TokenAddress, owner=caller_address, tokenId=_tokenId)
    custody.write(l1TokenAddress=_l1TokenAddress, tokenId=_tokenId, value=_l2TokenAddress)
    mintCredits.write(
        l1TokenAddress=_l1TokenAddress, tokenId=_tokenId, owner=caller_address, value=0)
...
func revokeMintCredit{
    ...
    send_message_to_l1(to_address=l1GatewayAddress, payload_size=5, payload=message_payload)
...
func warpToMainnet{
        syscall_ptr : felt*, storage_ptr : Storage*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _l1TokenAddress : felt, _tokenId : felt):
    let (caller_address) = get_caller_address()
